...
class MeshConv(nn.Module):
    ...
    def create_GeMM(self, x, Gi):
        ...
        # TODO: sanity check the gemm edge indexing is correct/order of dims is good
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_mat = torch.matmul(Q, K.permute(0, 1, 3, 2)) / np.sqrt(elem_size)
...
class MeshPoolSA(nn.Module):
    ...
    def forward(self, fe, meshes, attention_matrix):
        ...
        # NOTE: No need to reshape bc only 1 mesh at a time
        out_features = torch.cat(self.__updated_fe).unsqueeze(0)
    ...
    def has_boundaries(mesh, edge_id):
        # TODO: precompute mesh gemm_edges
        for edge in mesh.gemm_edges[edge_id]:
            if edge == -1 or -1 in mesh.gemm_edges[edge]:
                return True
        return False

    # NOTE: This checks the link condition for valid edge collapse!!
    @staticmethod
