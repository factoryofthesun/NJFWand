...
class Process:
    ...
    def deform(self, mesh):
        ...
        mod.angle = np.random.uniform(low=-0.3, high=0.3)
        mod.factor = np.random.uniform(low=-0.5, high=0.5)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generate deformed meshes')
    parser.add_argument('--sourcedir', type=str, help='path to folder of source meshes')
    parser.add_argument('--targetdir', type=str, help='path to folder for saving deformed meshes')
    parser.add_argument('--n', type=int, help='number of deformations to sample per mesh')
    ...
    for file in os.listdir(datadir):
        ...
        for i in range(n):
            n_def = np.random.choice(range(1,4))
